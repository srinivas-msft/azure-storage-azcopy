...
func verifyFileDirUpload(testFileCmd TestFileCommand) {
	sasUrl := testFileCmd.Subject
	// as it's a directory validation, regard the sasURL as a directory
	directoryClient, err := directory.NewClientWithNoCredential(sasUrl,nil)
	if err != nil {
		fmt.Printf("error creating directory client. failed with error %s\n", err.Error())
		os.Exit(1)
	}
	...
	if err != nil {
		fmt.Printf("error parsing url. failed with error %s\n", err.Error())
		os.Exit(1)
	}
...
func validateAzureDirWithLocalFile(curAzureDirURL *directory.Client, baseAzureDirPath string, localBaseDir string, isRecursive bool) {
	...
	for pager.More() {
		...
		for _, fileInfo := range listFile.Segment.Files {
			...
			// read all bytes.
			fileBytesDownloaded, err := io.ReadAll(get.Body)
			if err != nil {
				fmt.Printf("fail to read the body of file %s downloaded and failed with error %s\n", fileInfo.Name, err.Error())
				os.Exit(1)
			}

			uri, err := url.Parse(curFileURL.URL())
			if err != nil {
				fmt.Printf("fail to parse the url %s\n", curFileURL.URL())
				os.Exit(1)
			}
			tokens := strings.SplitAfterN(uri.Path, baseAzureDirPath, 2)
			if len(tokens) < 2 {
...
func verifySingleFileUpload(testFileCmd TestFileCommand) {
	...
	fileClient, err := file.NewClientWithNoCredential(testFileCmd.Subject, nil)
	if err != nil {
		fmt.Printf("error creating file client. failed with error %s\n", err.Error())
		os.Exit(1)
	}
	...
	if err != nil {
		fmt.Println("error mapping the destination file: ", localFile, " file size: ", fileInfo.Size(), " Error: ", err.Error())
		os.Exit(1)
	}
	...
	if !validateString(expectedContentType, common.IffNotNil(get.ContentType, "")) {
		str1 := fmt.Sprintf(" %s    %s", expectedContentType, common.IffNotNil(get.ContentType, ""))
