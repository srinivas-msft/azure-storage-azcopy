...
func cleanFile(resourceURL string) {
	fileClient := createFileClient(resourceURL)
	ctx := context.WithValue(context.Background(), ste.ServiceAPIVersionOverride, ste.DefaultServiceApiVersion)
...
func createFileClient(resourceURL string) *file.Client {
	fileURLParts, err := file.ParseURL(resourceURL)
	if err != nil {
		fmt.Println("Failed to parse url")
		os.Exit(1)
	}
	return createShareClient(resourceURL).NewRootDirectoryClient().NewFileClient(fileURLParts.DirectoryOrFilePath)
}
...
func createShareClient(resourceURL string) *share.Client {
	...
	if fileURLParts.ShareSnapshot != "" {
		...
		if err != nil {
			fmt.Println("Failed to create share with snapshot")
			os.Exit(1)
		}
	}
	return sc
...
func createFileServiceClient(resourceURL string) *fileservice.Client {
	...
	c, err := file.NewSharedKeyCredential(name, key)
	if err != nil {
		fmt.Println("Failed to create shared key credential!")
		os.Exit(1)
	}
...
func cleanFileAccount(resourceURL string) {
	...
	for pager.More() {
		...
		for _, shareItem := range lResp.Shares {
			_, err := serviceClient.NewShareClient(*shareItem.Name).Delete(ctx, nil)
			if err != nil {
				fmt.Println("error deleting the container from account, ", err)
				os.Exit(1)
			}
